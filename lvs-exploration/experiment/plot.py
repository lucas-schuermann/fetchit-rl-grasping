...
def smooth_reward_curve(x, y):
    halfwidth = int(np.ceil(len(x) / 100))
    k = halfwidth
    xsmoo = x
...
@click.command()
@click.argument('logs_dir', type=str)
def main(logs_dir):
    # Load all data.
    data = {}
    paths = [
        os.path.abspath(os.path.join(path, '..'))
        for path in glob2.glob(os.path.join(logs_dir, '**', 'progress.csv'))
    ]
    for curr_path in paths:
        ...
        z = mean_Q
        x, z = smooth_reward_curve(epoch, mean_Q)
        ...
        for config in sorted(data[env_id].keys()):
            ...
            plt.savefig(
                os.path.join(logs_dir,
                             'fig_{}.png'.format(env_id + '-Qvalues')))

            plt.subplot(212)
            plt.plot(xs[0], np.nanmedian(zs, axis=0), label=config)
            plt.fill_between(xs[0],
                             np.nanpercentile(zs, 25, axis=0),
                             np.nanpercentile(zs, 75, axis=0),
                             alpha=0.25)
            plt.xlabel('Epoch')
            ...
            plt.savefig(
                os.path.join(logs_dir,
                             'fig_{}.png'.format(env_id + '-success_rate')))

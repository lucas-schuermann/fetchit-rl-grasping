...
def train(policy, rollout_worker, evaluator, n_epochs, n_test_rollouts,
    ...
    for epoch in range(n_epochs):
        ...
        logger.info("Best success rate so far ", best_success_rate,
                    " In epoch number ", best_success_epoch)
...
def launch(env,
    ...
    #resource.setrlimit(resource.RLIMIT_NOFILE, (65536/4, 65536/4))
    ...
    rollout_worker = RolloutWorkerOriginal(params['make_env'], policy, dims,
                                           logger, **rollout_params)
    rollout_worker.seed(rank_seed)

    evaluator = RolloutWorkerOriginal(params['make_env'], policy, dims, logger,
                                      **eval_params)
...
@click.option(
    '--env',
    type=str,
    default='LVSFetchPickAndPlace-v1',
...
@click.option('--clip_return',
              type=int,
              default=1,
              help='whether or not returns should be clipped')
@click.option('--demo_file',
              type=str,
              default='data_pick_random_100.npz',
